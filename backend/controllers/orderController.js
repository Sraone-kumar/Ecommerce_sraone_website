...
const getOrder = async (req, res, next) => {
  try {
    const order = await Order.findById(req.params.id)
      .populate("user", "-password -isAdmin -_id -_v -createdAt -updatedAt")
      .orFail();
...
const createOrder = async (req, res, next) => {
  try {
    ...
    if (!cartItems || !orderTotal || !paymentMethod) {
      return res.status(400).send("missing one or more required inputs");
    }
...
const updateOrderToDelivered = async (req, res, next) => {
  try {
    ...
    const deliveredUpdated = await order.save();
    res.send(deliveredUpdated);
  } catch (error) {
    next(error);
  }
};
